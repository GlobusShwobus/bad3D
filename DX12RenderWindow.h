...
public:
	...
	bool is_size_out_of_sync()const
	{
		// query the true size of the client
		const UIRect rect = get_client_rect(mHwnd);

		return mBufferData.mWidth != rect.w || mBufferData.mHeight != rect.h;
	}
	...
	{
		return get_buffer_desc_at(mBufferData.mCurrentIndex);
	}
	UINT64 get_current_buffer_index()const { return mBufferData.mCurrentIndex; }
