...
LRESULT Application::on_message(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
		...
		{
			UINT cwidth, cheight;
			mDXWindow->get_client_size(cwidth,cheight);

			if (mDXWindow->needs_resize(cwidth,cheight))
			{
				mDXWindow->on_resize(cwidth,cheight);
				mGame->on_window_resize(cwidth,cheight);
			}
