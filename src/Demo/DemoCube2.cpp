...
{
	// gets
	mGraphicsDevice = ViewPtr{ device };
	mDevice = ViewPtr{ device->get_device() };
	mRenderWindow = ViewPtr{ window };

	auto copy_command_queue = device->get_copy_queue();
	...
	copy_command_queue->wait_until_completion(
		copy_command_queue->execute(std::move(copy_command_list))
	);

	//other
	mRenderWindow->set_clear_color(color[0], color[1], color[2], color[3]);
	...
	mViewport = D3D12_VIEWPORT{ 0.0f, 0.0f, static_cast<float>(mRenderWindow->get_buffer_width()), static_cast<float>(mRenderWindow->get_buffer_height()), 0.0f, 1.0f };
	...
	resize_depth_buffer(mRenderWindow->get_buffer_width(), mRenderWindow->get_buffer_height());
...
void DemoCube2::on_update()
{
	...
	if (mRunning)
	{
		...
		UINT client_width = mRenderWindow->get_buffer_width();
		UINT client_height = mRenderWindow->get_buffer_height();
...
void DemoCube2::on_render()
{
	if (mRunning)
	{
		// find a command list either via render window helper or caching queue directly, makes not a lot of difference atm (later queue should accept batch work)
		auto command_context = mRenderWindow->get_command_list();
		...
		// output merger state
		auto buffer_desc = mRenderWindow->get_buffer_desc();
		...
		mRenderWindow->submit_work(std::move(command_context));
	}
}
...
void DemoCube2::on_resize(int w, int h)
{
	...
	const UINT buffer_width = mRenderWindow->get_buffer_width();
	const UINT buffer_height = mRenderWindow->get_buffer_height();
...
void DemoCube2::kb_resolve()
{
	...
	if (f11_current && !f11_previous)
	{
		...
		mRenderWindow->toggle_fullscreen(fullscreen);
	}
